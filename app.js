...
const mobileQuickJump = document.getElementById('mobileQuickJump');
const mobileNewTradeBtn = document.getElementById('mobileNewTradeBtn');
const mobileAnalyticsBtn = document.getElementById('mobileAnalyticsBtn');
const mobileQuickTrade = document.getElementById('mobileQuickTrade');
const mobileAccountBtn = document.getElementById('mobileAccountBtn');
const exportBtn = document.getElementById('exportBtn');
const mobileBottomNavButtons = Array.from(document.querySelectorAll('.mobile-bottom-nav button'));
const stickyActionButtons = Array.from(document.querySelectorAll('.sticky-mobile-actions button'));
...
function toggleSidebar(forceOpen) {
  const shouldOpen = typeof forceOpen === 'boolean'
    ? forceOpen
    : !(body.classList.contains('sidebar-open') || body.classList.contains('mobile-nav-open'));

  setBodyNavState(shouldOpen);

  if (sidebar && isMobileView()) {
    sidebar.style.transform = shouldOpen ? 'translateX(0)' : 'translateX(-105%)';
  }
}
...
function openAccountPanel() {
  ...
  if (authPanel) {
    requestAnimationFrame(() => authPanel.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  }
}

function focusJournalFirstField() {
  const field = document.getElementById('instrument');
  if (!field) return;
...
function updateMobileSummary() {
  ...
  const totalPL = formatCurrency(closed.reduce((sum, t) => sum + Number(t.pl || 0), 0));

  const mobileTrades = document.getElementById('mobileTradesCount');
  const mobileWinRate = document.getElementById('mobileWinRate');
  const mobileNetPL = document.getElementById('mobileNetPL');

  if (mobileTrades) mobileTrades.textContent = String(closed.length);
  if (mobileWinRate) mobileWinRate.textContent = winRate;
  if (mobileNetPL) mobileNetPL.textContent = totalPL;
}
...
function drawLineChart(canvasId, data, options = {}) {
  ...
  ctx.beginPath();
  ctx.lineWidth = isMobileView() ? 2.25 : 3;
  ...
  data.forEach((value, index) => {
    ...
    ctx.arc(x, y, isMobileView() ? 3 : 4, 0, Math.PI * 2);
    ctx.fill();
  });
}
...
function drawBarChart(canvasId, items, options = {}) {
  ...
  items.forEach((item, index) => {
    ...
    ctx.font = isMobileView() ? '11px Arial' : '12px Arial';
...
function drawMonthlyBars(canvasId, items) {
  const series = items.length
    ...
    : [{ label: 'M1', value: 0 }];
  drawBarChart(canvasId, series, { color: '#f0cc84', negativeColor: '#8eb7ff', height: isMobileView() ? 260 : 320 });
}
...
function drawAllCharts() {
  drawLineChart('equityChart', equityData.length ? equityData : [10000], { color: '#d9b56d', point: '#f0cc84', height: isMobileView() ? 260 : 380 });
  drawBarChart('sessionChart', sessionData.length ? sessionData : [{ label: 'London', value: 0 }], { color: '#d9b56d', negativeColor: '#8eb7ff', height: isMobileView() ? 240 : 320 });
  drawLineChart('drawdownChart', drawdownData.length ? drawdownData : [0], { color: '#8eb7ff', point: '#8eb7ff', height: isMobileView() ? 240 : 320 });
  drawBarChart('setupChart', setupData.length ? setupData : [{ label: 'Retest', value: 0 }], { color: '#f0cc84', negativeColor: '#8eb7ff', height: isMobileView() ? 240 : 320 });
  drawLineChart('marketChart', marketPrices.length ? marketPrices : [0], { color: '#d9b56d', point: '#f0cc84', height: isMobileView() ? 260 : 380 });
...
function switchTab(target) {
  ...
  if (!resolvedTarget || !document.getElementById(resolvedTarget)) {
    resolvedTarget = 'home';
  }

  tabs.forEach(button => button.classList.toggle('active', button.dataset.tab === resolvedTarget));
  panels.forEach(panel => panel.classList.toggle('active', panel.id === resolvedTarget));

  const activeTab = document.querySelector(`.nav-tab[data-tab="${resolvedTarget}"]`);
  const label = activeTab ? activeTab.textContent.trim() : resolvedTarget.charAt(0).toUpperCase() + resolvedTarget.slice(1);

  if (pageTitle) pageTitle.textContent = label;
  if (mobilePageTitle) mobilePageTitle.textContent = label;
  if (mobileQuickJump) mobileQuickJump.value = resolvedTarget;

  mobileBottomNavButtons.forEach(button => {
    button.classList.toggle('active', button.dataset.tab === resolvedTarget);
  });

  stickyActionButtons.forEach(button => {
    button.classList.toggle('active', button.dataset.tab === resolvedTarget);
  });
...
function handleExport() {
  const sorted = sortTradesDesc(trades);
  downloadFile('edge-trading-journal-trades.json', JSON.stringify(sorted, null, 2), 'application/json');
}
...
[newTradeBtn, openJournalBtn, mobileNewTradeBtn, mobileQuickTrade].forEach(button => {
  if (button) {
    button.addEventListener('click', event => {
      event.preventDefault();
      switchTab('journal');
      focusJournalFirstField();
    });
  }
});

[mobileAnalyticsBtn, viewAnalyticsBtn].forEach(button => {
  if (button) {
    button.addEventListener('click', event => {
      event.preventDefault();
      switchTab('analytics');
...
if (mobileQuickJump) {
  mobileQuickJump.addEventListener('change', event => {
...
if (mobileMenuToggle) {
  ['click', 'touchend'].forEach(eventName => {
    mobileMenuToggle.addEventListener(eventName, event => {
      event.preventDefault();
...
if (mobileSidebarOverlay) {
  ...
  mobileSidebarOverlay.addEventListener('touchend', closeSidebar, { passive: true });
}

if (mobileSidebarBackdrop) {
  mobileSidebarBackdrop.addEventListener('click', closeSidebar);
  mobileSidebarBackdrop.addEventListener('touchend', closeSidebar, { passive: true });
}
...
document.addEventListener('click', event => {
  const actionable = event.target.closest('[data-tab]');
  if (!actionable) return;
  const tab = actionable.getAttribute('data-tab');
  if (!tab || actionable.classList.contains('nav-tab')) return;
  switchTab(tab);
});
...
refreshAuthUI();
updateResponsiveTables();
switchTab(document.querySelector('.nav-tab.active')?.dataset.tab || 'home');
