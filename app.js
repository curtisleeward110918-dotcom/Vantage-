...
function switchTab(target) {
  ...
  const mobileBottomButtons = document.querySelectorAll('.mobile-bottom-nav button');

  if (activeTab) activeTab.classList.add('active');
  if (targetPanel) targetPanel.classList.add('active');
  if (pageTitle && activeTab) pageTitle.textContent = activeTab.textContent;
  if (mobilePageTitle && activeTab) mobilePageTitle.textContent = activeTab.textContent;

  mobileBottomButtons.forEach(button => {
    button.classList.toggle('active', button.dataset.tab === target);
  });

  if (mobileQuickJump) mobileQuickJump.value = target;
...
[newTradeBtn, openJournalBtn, mobileNewTradeBtn].forEach(button => {
  if (button) {
...
[mobileAnalyticsBtn, viewAnalyticsBtn].forEach(button => {
  if (button) button.addEventListener('click', () => switchTab('analytics'));
});
...
document.querySelectorAll('[data-close-mobile-nav="true"]').forEach(el => {
  el.addEventListener('click', closeSidebar);
});

document.querySelectorAll('.mobile-bottom-nav button, .sticky-mobile-actions button').forEach(button => {
  button.addEventListener('click', () => {
    const target = button.dataset.tab;
    if (!target) return;
    switchTab(target);
    if (target === 'journal') focusJournalFirstField();
  });
});

const mobileQuickTrade = document.getElementById('mobileQuickTrade');
if (mobileQuickTrade) {
  mobileQuickTrade.addEventListener('click', () => {
    switchTab('journal');
    focusJournalFirstField();
  });
}
